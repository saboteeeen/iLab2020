...
class RnnlmTrainer:
    ...
    def fit(self, xs, ts,max_epoch=1, batch_size=20, time_size=35,
        ...
        for epoch in range(max_epoch):
            ...
            ave = 0
            for iters in range(max_iters):
                self.batch_x, self.batch_t = self.get_batch(xs, ts, batch_size, time_size)
                # 勾配を求め、パラメータを更新
                ...
                if (eval_interval is not None) and (iters % eval_interval) == 0:
                    ...
                    total_loss, loss_count = 0, 0
            self.current_epoch += 1# 
            while len(values) == int((max_iters / eval_interval)+1):#
                for i in range(len(values) - 1):#
                    sub.append(values[i] - values[i+1])#
                for j in range(len(sub)):#
                    sum_res += abs(sub[j])#
                d_ave = sum_res / int((max_iters / eval_interval)+1)#
                print(d_ave)
                return d_ave               
